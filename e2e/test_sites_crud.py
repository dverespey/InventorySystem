...
markers (grep wrapper.log). Per-check PASS/FAIL/SKIP; exit 1 if any FAIL.
  ...
  python3 e2e/test_sites_crud.py            # headless gate
  python3 e2e/test_sites_crud.py --headed   # watch it live
...
SITES-MASTER-SPECIFIC (differs from the other masters):
  RULE #1 ROLE-GATED — SERVER-SIDE WRITE GATE ONLY (M4 auth fix 2026-06-22; UI gate
    ...
    proven) by e2e/test_m4_auth.py driving the EXACT deployed Save/Delete scripts.
...
db/namedqueries/master-crud-namedqueries.sql + spike-inv-sites-table.sql):

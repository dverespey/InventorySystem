...
environment or a gitignored e2e/.env (KEY=VALUE lines).
"""
import os, re, subprocess, sys, time

# Centralized gateway-path + DB-connection constants (repo-split-plan §4.C/§4.D). `_ignenv` lives in
# generators/ (a sibling of e2e/ under the repo root); add it to the path so the whole harness shares
# ONE env-parametrized source of truth (gateway root/project dir + the Inventory_Spike->Inventory rename
# point). Defaults are the current spike values, so nothing changes at runtime unless an IGN_* env var
# is set.
...
def _register_db_shared():
    ...
    code = os.path.normpath(os.path.join(
        ...
        "ignition", "project-library", "db_shared", "code.py"))
...
def load_env():
    """Populate os.environ from e2e/.env if present (no override of real env)."""

...
session-scoped prop that persists across views in the same browser session), then navigate to the view.
  ...
  python3 scripts/e2e/master_detail_layout_render.py            # headless, writes artifacts/*.png
  python3 scripts/e2e/master_detail_layout_render.py --headed   # watch it
"""
import sys
import time
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if "Trial Expired" in page.inner_text("body"):
            print("PERSPECTIVE TRIAL EXPIRED — run scripts/e2e/reset_trial.py first")
            b.close()
            sys.exit(2)

...
type
  TData_Module = class(TDataModule)
    ...
    INV_StoredProc1: TADOStoredProc;
    fiTemplateDir: TCIniField;
    fiUseApplicationDir: TCIniField;
    fiCreatePOPriorToClose: TCIniField;
    SiteTMMDUNSDataSet: TADODataSet;
...
//11-14-2002
    ...
    fFilmPartNum:string;
    fLabelPartNum:string;
    fMisc1PartNum:string;
    fMisc2PartNum:string;
...
//11-14-2002
    ...
    property LabelPartNum: string read fLabelPArtNum write fLabelPartNum;
    property Misc1PartNum: string read fMisc1PartNum write fMisc1PArtNum;
    property Misc2PartNum: string read fMisc2PartNum write fMisc2PArtNum;
    ...
    function UpdateUserPassword(UserID:string; Password:string):boolean;
...
function TData_Module.TemplateDir:String;
begin
  //add ending slash to keep consistant
  if fiUseApplicationDir.AsBoolean then result:=ExtractFilePath(application.ExeName) else result:=fiTemplateDir.AsString+'\';
end;
...
function TData_Module.GetLines():TStringList;
var
  lines:TStringList;
begin
  result:=nil;
  ...
  try
    ...
    begin
      ...
      while not EOF do
      begin
        lines.Add(fieldByName('LineName').AsString);
        next;
      end;
    ...
    on e:exception do
    Begin
        LogActLog('ERROR', 'Unable to get ASN line information. Err Msg: ' + E.Message + ' Err: ' + E.ClassName)
    End;      //Except
  end;
end;
  ...
  try
    try
      ...
      Begin
        ...
        Parameters.AddParameter.Name := '@LabelCode';
        Parameters.ParamValues['@LabelCode'] := fLabelPartNum;
        Parameters.AddParameter.Name := '@Misc1Code';
        Parameters.ParamValues['@Misc1Code'] := fMisc1PartNum;
        Parameters.AddParameter.Name := '@Misc2Code';
        Parameters.ParamValues['@Misc2Code'] := fMisc2PartNum;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        fEndDate:=fieldbyname('EndSeqDT').AsDateTime;
        with ALC_Dataset do
        begin
          Close;
          CommandType := CmdStoredProc;
...
begin
  ...
  try
    ...
    begin
      ...
      // remove inventory
      if length(Inv_StoredProc.FieldByName('VC_TIRE_PART_NUMBER1_CODE').AsString) > 2 then
      begin
      ...
      if length(Inv_StoredProc.FieldByName('VC_TIRE_PART_NUMBER2_CODE').AsString) > 2 then
      begin
      ...
      if length(Inv_StoredProc.FieldByName('VC_WHEEL_PART_NUMBER1_CODE').AsString) > 2 then
      begin
      ...
      if length(Inv_StoredProc.FieldByName('VC_WHEEL_PART_NUMBER2_CODE').AsString) > 2 then
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Inv_StoredProc.Parameters.ParamValues['@BCode'] := TRIM(FieldByName('BC').AsString);
              ...
              begin
                ...
                if i < 2 then
                begin
                  ShowMessage('Missing Broadcast Code Information('+TRIM(FieldByName('BC').AsString)+'), inventory pull failed');
                  LogActLog('ERROR','Missing Broadcast Code Information('+TRIM(FieldByName('BC').AsString)+'), inventory pull failed');
                  raise EDatabaseError.Create('Missing Broadcast Code Information('+TRIM(FieldByName('BC').AsString)+'), inventory pull failed');
                end;
              end;
            end;
            next; // get next BC
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Inv_StoredProc.recordcount > 0 then
          begin
            // Check for manifest cost information
            while not Inv_StoredProc.eof do
            begin
              errorstr:='';
              if (inv_StoredProc.FieldByName('IN_MANIFEST_COST_ID').IsNull) then
              begin
                errorstr:=errorstr+' '+Inv_StoredProc.FieldByName('VC_ASSY_PART_NUMBER_CODE').AsString+ ' ';
              end;
              Inv_StoredProc.next;
            end;
            ...
            if (errorstr <> '') then
            begin
              ShowMessage('Missing Manifest Cost Information BCode('+ALC_StoredProc.FieldByName('BC').AsString+') Assy Part Number ('+errorstr+')ASN create failed');
              LogActLog('ERROR','Missing Manifest Cost Information BCode('+ALC_StoredProc.FieldByName('BC').AsString+') Assy Part Number ('+errorstr+')ASN create failed');
              raise EDatabaseError.Create('Missing Manifest Cost Information BCode('+ALC_StoredProc.FieldByName('BC').AsString+') Assy Part Number ('+errorstr+')ASN create failed');
            end;

            Inv_StoredProc.First;
            ...
            begin
              ...
              begin
                ...
                count:=FieldByName('VEHICLES').AsInteger * Inv_StoredProc.FieldByName('IN_ASSY_QTY').AsInteger;
                ...
                LogActLog('ASN', 'INSERT ASN entry(No Ratio) ASNID('+IntToStr(fRecordID)+') Manifest('+manifest+') Qty('+IntToStr(count)+')');
                break;
              end
              else
              begin
                ...
                //
                //
                //  Modify to support more then three shares
                //
                //  To support forecast the share for tire and wheel will be set to the same quantity
                //  For here we'll use only one of the values for the ASN share amount
                //

                if (Inv_StoredProc.FieldByName('IN_TIRE_RATIO').AsInteger = 100) and
                                  (Inv_StoredProc.FieldByName('IN_WHEEL_RATIO').AsInteger = 100) then
                begin
                  count:=FieldByName('VEHICLES').AsInteger * Inv_StoredProc.FieldByName('IN_ASSY_QTY').AsInteger;
                end
                ...
                begin
                  count:=round((FieldByName('VEHICLES').AsInteger  * Inv_StoredProc.FieldByName('IN_ASSY_QTY').AsInteger *
                    Inv_StoredProc.FieldByName('IN_TIRE_RATIO').AsInteger) / 100);
                end;
        ...
        //Add Manifest Cost Check for all inserted ASN Detail entries
        //
        //  Use fRecordID and query, if any data returned then those are missing cost info
        //  missing cost can be no entry of out of date entry.

        try
          Inv_StoredProc.Close;
          Inv_StoredProc.ProcedureName := 'dbo.SELECT_ASNMissingCost;1';
          Inv_StoredProc.Parameters.Clear;
          Inv_StoredProc.Parameters.AddParameter.Name := '@ASNID';
          Inv_StoredProc.Parameters.ParamValues['@ASNID'] := fRecordID;
          Inv_StoredProc.Open;
          ...
          begin
              ...
              begin
                // missing bc code fail on pull
                ShowMessage('Part Number('+Inv_StoredProc.FieldByName('PartNumber').AsString+') with Manifest Number('+Inv_StoredProc.FieldByName('Manifest').AsString+') has; '+Inv_StoredProc.FieldByName('ErrorMsg').AsString);
                LogActLog('ERROR','Part Number('+Inv_StoredProc.FieldByName('PartNumber').AsString+') with Manifest Number('+Inv_StoredProc.FieldByName('Manifest').AsString+') has; '+Inv_StoredProc.FieldByName('ErrorMsg').AsString);
                next;
              end;
          end;
        except
          on e:exception do
          begin
            LogActLog('ERROR','Failed ASNMissingCost check, '+e.message);
          end;
        end;
      end;
    end;
    ...
    on e:exception do
    begin
      LogActLog('ERROR','Failed on ASN Items Create, '+e.message);
      result:=False;
    end;
  end;
end;
      ...
      Begin
        ...
        begin
          ...
          Begin
            ...
            if ((Inv_StoredProc1.FieldByName('UPDATEDIFF').AsInteger >= Inv_StoredProc1.FieldByName('IN_PASSWORD_RESET_DAYS').AsInteger)
                    OR Inv_StoredProc1.FieldByName('UPDATEDIFF').IsNull) then
            begin
                ShowMessage('Your password has expired');

                NewPasswordDlg:=TNewPasswordDlg.Create(self);
                NewPAsswordDlg.Execute;

                if (NewPAsswordDlg.Cancel) then
                begin
                  Result := False;
                  fTrans := 'LOGIN ERR';
                ...
                begin
                  // Set New Password
                  if UpdateUserPAssword(fUSerID, NewPasswordDlg.Password) then
                  begin
                    Result := True;
                    fTrans := 'LOGIN';
...
function TData_Module.UpdateUserPassword(UserID:string; PAssword:string):boolean;
var
  fDescription: String;
begin
  try
    ...
    With Inv_StoredProc do
    Begin
      ProcedureName := 'dbo.UPDATE_UserPAssword;1';
      Parameters.Clear;
      Parameters.AddParameter.Name := '@UserID';
      Parameters.ParamValues['@UserID'] := UserID;
      Parameters.AddParameter.Name := '@Password';
      Parameters.ParamValues['@Password'] := Password;

      fBeforeDateTime := Time;
      ExecProc;
      if Inv_Connection.Errors.Count > 0 then
      begin
        ShowMessage('Unable to update User Password, '+Inv_Connection.Errors.Item[0].Get_Description);
        LogActLog('ERROR','Unable to update User Password, '+Inv_Connection.Errors.Item[0].Get_Description);
        raise EDatabaseError.Create('Unable to update User Password');
      end
      else
      begin
        fAfterDateTime := Time;
    ...
    End;    //With
    LogActLog('UPD USER PW', fDescription, 1);
    result:=TRUE;
  except
    on e:exception do
    Begin
...
function TData_Module.OpenPOOrders:boolean;
begin
  //Find any unprocessed orders and report
  result:=FALSE;

  try
    Inv_StoredProc.Close;
    Inv_StoredProc.ProcedureName := 'dbo.SELECT_OrderNotOrdered;1';
    Inv_StoredProc.Parameters.Clear;
    Inv_StoredProc.Open;
    if Inv_StoredProc.RecordCount > 0 then
    begin
    ...
    on e:exception do
    Begin
      LogActLog('ERROR', 'FAILED SELECT Open PO Check. Err Msg: ' + E.Message + ' Err: ' + E.ClassName)
    End;      //Except
  end;
end;

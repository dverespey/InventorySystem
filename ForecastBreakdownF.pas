...
type                                       
  ...
  private
    ...
    //fEntries:array[0..100] of TEntryRec;
...
begin
    ...
    Begin
      ...
      Parameters.ParamValues['@WeekDate'] := fFirstWeekDate;
...
procedure TForecastBreakdown_Form.splitString(delim: char; s: string; ListOfStrings: TStrings);
var temp: string;
    i: integer;
begin
   ListOfStrings.Clear;
...
begin
  ...
  try
    ...
    begin
      splitString('*',fcl,delSL);
      Hist.Append('Trading Partner Search:'+delSL[4]);

      Data_Module.SiteTMMDUNSDataset.Close;
      Data_Module.SiteTMMDUNSDataset.Parameters.Clear;
      Data_Module.SiteTMMDUNSDataset.Parameters.AddParameter.Name := '@SiteTMMDUNS';
      Data_Module.SiteTMMDUNSDataset.Parameters.ParamValues['@SiteTMMDUNS'] := delSL[4];
      Data_Module.SiteTMMDUNSDataset.Open;

      if Data_Module.SiteTMMDUNSDataset.RecordCount = 0 then
      begin
        Hist.Append('Trading Partner Search: Not Found '+delSL[4]);
    ...
    Hist.Append('EDI Count='+IntToStr(count));
    Data_Module.LogActLog('FORECAST','EDI Count='+IntToStr(count));

    reset(fcf);
    SetLength(fEntries,count);
    Hist.Append('After Set Length');
    ...
    try
      ...
      begin
        ...
        begin
          ...
          splitString('*',fcl,delSL);
          //LIN Entry
          fEntries[counter].Supplier:=Data_Module.fiSupplierCode.AsString;
          fEntries[counter].Partnumber:=delSL[3];
          fEntries[counter].KanbanNumber:=delSL[5];
          fEntries[counter].Skip:=False;
          ...
          begin
            ...
            fEntries[counter].Weeks[weeks].WeekNumber:=StrToInt(copy(delSL[9],3,2));
            fEntries[counter].Weeks[weeks].WeekDate:=delSL[4];
            if (weeks=1) then
            begin
              fFirstWeekDate:=fEntries[counter].Weeks[weeks].WeekDate;
              fFirstWeekNumber:=fEntries[counter].Weeks[weeks].WeekNumber;
            end;
            fEntries[counter].Weeks[weeks].WeekCount:=StrToInt(delSL[1]);
...
begin
  ...
  Hist.append(IntToStr(high(fEntries)+1-skip)+' forecast records to be added');
  Data_Module.LogActLog('FORECAST',IntToStr(high(fEntries)+1-skip)+' forecast records to be added');
end;
      ...
      try
        for j:=1 to count do
          ...
          Begin
            ...
            Parameters.AddParameter.Name := '@WeekDate';
            Parameters.ParamValues['@WeekDate'] := fEntries[i].Weeks[j].WeekDate;
          ...
          try
            ...
            begin
              ...
              begin
                //
                //  Change to reflect TEMA forecast of shared parts
                //
                //  TEMA sends a forecast line for each item of the share with the full amount summed from all lines
                //
                //  EX share on ties of 60/40 1000 total, TEMA sends two lines with 1000 each
                //  if share is three ways 20/40/40 then TEMA sends three lines with 1000 each
                //  Proper way is to configure share values in tire, then split the non share either 50/50 for 2 or 33/33/33 for three
                //
                //  The way this code is designed there is no way to know how many lines to expect so use config
                //
              ...
              if length(FieldByName('Tire Part Number Code').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Wheel Part Number Code').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Valve Part Number').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Film Part Number').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Label Part Number').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Misc1 Part Number').AsString) > 2 then
              begin
              ...
              if length(FieldByName('Misc2 Part Number').AsString) > 2 then
              begin
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Parameters.ParamValues['@ProdYear'] := copy(weekdate,1,4);// formatdatetime('yyyy',EventDate_NUMMIBmDateEdit.date);
          Open;
    ...
    Begin
      ...
      Parameters.AddParameter.Name := '@WeekDate';
      Parameters.ParamValues['@WeekDate'] := WeekDate;
      Parameters.AddParameter.Name := '@Supplier';
